#!/bin/bash
# CD-ViTO SIXray-D exact-K-instance queue, seeds 2..10 x 1/5/10-shot (27 jobs
# by default). Mirrors run_cdvito_sixray_exact_clean.sh (seed1) but loops
# over seeds and writes to a separate output tree so seed1's results are
# never touched.
#
# PREREQUISITE: build_prototypes_sixray_exact_multiseed.sh must have been run
# first for the same seeds.
#
# Usage: ./run_cdvito_sixray_exact_multiseed.sh [SEED...]  (default: 2..10)
set -uo pipefail
...
RESULT_DIR="output/vitl_sixray_exact_multiseed"
...
for seed in "${SEEDS[@]}"; do
    for shot in "${SHOTS[@]}"; do
        name="sixray_exact_${shot}shot_seed${seed}"
        config_file="configs/sixray_exact_instance_seed1_clean/vitl_shot${shot}_sixray_finetune_seed${seed}.yaml"
        proto_file="prototypes_init/sixray_exact_train_${shot}shot_seed${seed}.vitl14.bbox.p${shot}.sk.pkl"
