#!/bin/bash
# FT-FSOD SIXray-D exact-K-instance queue, seeds 2..10 x 1/5/10-shot (27 jobs
# by default). Uses configs_cdfsod/final_configs_sixray_exact_seed1_clean/
# (val!=test, matching the seed1 clean protocol). Writes to
# exp_sixray_exact_multiseed/ - existing exp_sixray_exact_seed1_clean/
# (seed1) is never touched. This is the single most expensive stage
# (~5hr/job measured on seed1) - runs last in the overall chain.
#
# Usage: ./run_ftfsod_sixray_exact_multiseed.sh [SEED...]  (default: 2..10)
...
RESULT_DIR="exp_sixray_exact_multiseed"
GPUID="0"
PORT="9999"
SHOTS=(1 5 10)
...
for seed in "${SEEDS[@]}"; do
    for shot in "${SHOTS[@]}"; do
        name="SIXray-D_${shot}shot_seed${seed}"
